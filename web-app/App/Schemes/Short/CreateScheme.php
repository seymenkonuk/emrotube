...
namespace Seymen\PhpMvcTemplate\Schemes\Short;
        ...
        return [
            "thumbnail_photo" => [
                ...
                "size" => Validator::create()
                    ->int()
                    ->min(ValidationConfig::MIN_THUMBNAIL_FILE_SIZE)
            ...
            "video_file" => [
                "name" => Validator::create()
                    ...
                    ->min(ValidationConfig::FILE_NAME_MIN_LEN)
                    ->max(ValidationConfig::FILE_NAME_MAX_LEN)
                    ->insertRegexRule(ValidationConfig::SHORT_FILE_NAME_REGEX_ERROR, ValidationConfig::SHORT_FILE_NAME_REGEX_RULE)
                    ->require(),
                "type" => Validator::create()
                    ->str()
                    ->insertAllowValues(ValidationConfig::ALLOW_SHORT_FILES)
                    ->require(),
                "tmp_name" => Validator::create()
                    ->str()
                    ->require(),
                ...
                "size" => Validator::create()
                    ->int()
                    ->min(ValidationConfig::MIN_SHORT_FILE_SIZE)
                    ->max(ValidationConfig::MAX_SHORT_FILE_SIZE)
                    ->require(),
                "full_path" => Validator::create()
                    ->str()
                    ->optional(),
